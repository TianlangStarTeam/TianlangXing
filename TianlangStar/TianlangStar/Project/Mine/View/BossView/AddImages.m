...
@interface AddImages ()<UINavigationBarDelegate,UIImagePickerControllerDelegate>
{
    // 标识被编辑的按钮 -1 为添加新的按钮
    NSInteger editTag;
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        UIButton *button = [self createButtonWithImage:kAddImage andSeletor:@selector(addNewImage:)];
...
{
     [self.imagesArray removeObject:[(UIButton *)btn.superview imageForState:UIControlStateNormal]];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    else
    {
        ...
        NSInteger index = [self.imagesArray indexOfObject:[btn imageForState:UIControlStateNormal]];
...
- (void)addNewImage:(UIButton *)button
{
    // 标识为添加一个新的图片
    if (![self deleClose:button
          ])
    {
        editTag = -1;
