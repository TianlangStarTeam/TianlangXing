...
//存贮的文件名
#define HSFileNa(url) [NSString stringWithFormat:@"%@",url.md5String]
...
- (NSInteger)fileTotalLength:(NSString *)url
{
    ...
    NSDictionary *dict =[NSDictionary dictionaryWithContentsOfFile:HSTotalLengthFullpath][self.title];
    return [dict[@"视频长度"] integerValue];
...
- (void)deleteFile:(NSString *)url
{
    ...
    if ([fileManager fileExistsAtPath:HSFileFullpath(url)]) {
        ...
        if ([fileManager fileExistsAtPath:HSTotalLengthFullpath]) {
            ...
            [dict removeObjectForKey:self.title];
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)dataTask didReceiveResponse:(NSHTTPURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler
{
    ...
    NSDictionary *dict1 = @{@"视频名":HSFileNa((sessionModel.url)),@"视频长度":@(totalLength),@"title":self.title,@"url":self.url};

    [dict setObject:dict1 forKey:self.title];
